...
template<typename T>
void GetQuadrangleSubPix(const T* src, size_t src_step, cv::Size src_size,
                                   T* dst, size_t dst_step, cv::Size win_size,
                                   const double *matrix, int cn)
{
    ...
    for (y = 0; y < win_size.height; y++, dst += dst_step)
    {
        ...
        for (x = 0; x < win_size.width; x++)
        {
            ...
            int ixs = cvRound(xs);
            int iys = cvRound(ys);
            const T* ptr = src + src_step * iys + ixs * cn;
...
cv::Mat GetQuadrangleSubPix(const cv::Mat& src, cv::Mat& dst, cv::Mat& m)
{
    ...
    if(src.type() == CV_32FC3){
        GetQuadrangleSubPix((float *)src.data, src.step, src.size(),
                                        (float *)dst.data, dst.step, dst.size(),
                                        matrix, src.channels());
    }else if(src.type() == CV_8UC3){
        GetQuadrangleSubPix(src.data, src.step, src.size(),
...
cv::Mat GetRotRectImg(cv::RotatedRect rr, cv::Mat& img, cv::Mat& dst)
{
    ...
    return GetQuadrangleSubPix(img, dst, m);
}
